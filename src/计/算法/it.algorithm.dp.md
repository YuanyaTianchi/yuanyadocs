...
title = "it.algorithm.dp"
description = "it.algorithm.dp"
tags = ["it", "algorithm"]
...
```go
// 零钱最小个数。找到问题：amount 作状态，coins 作选项，dp[n] 表示 amount=n 时的零钱最小个数
// 是否存在重复子问题？假设 amount=5，coins={1,3}，选项序列为 3 或 111 时，amount 都余 2，存在重复子问题
// 思路：先遍历状态 amount，然后遍历选项，大于 amount 时返回，等于 amount 时组合数 count++
// 逻辑表示由最开始的已知基础情况解 dp[0]=0 的解得到 dp[1] 的解，依次类推得到 dp[n]
// 状态转移表达式：dp[n] = min(dp[n], dp[n-coin]+1)。n-coin 表示选择了当前选项 coin
func coinChange(coins []int, amount int) int {
	...
	dp[0] = 0
	// 元素初始值赋为amount+1，以便直接使用状态转移方程，否则无法直接取 min(dp[i], dp[i-coin]+1)
	for i := 1; i < len(dp); i++ {
		dp[i] = amount + 1
	}
...
```go
// 零钱组合数。找到问题：amount 作状态，coins 作选项，dp[n] 表示 amount=n 时的零钱组合数
// 假如带入最小零钱个数问题的思路，发现存在无法解决的重复组合问题，比如选项序列 522、252、225 属于相同组合
// 思路：先遍历选项 coins，相当于由 coins={1} 时得到的 dp[n] 的解，作为 coins={1,3} 时 dp[n] 的子解
// 至于如何得到 coins={1} 时的 dp[n] 的解，通过常规逻辑由基础情况解 dp[0]=1 开始，类推直到 dp[n] 即可
// 这里 dp[0]=1，因为 amount=0 时，只有不选择任何零钱这一个组合，这也属于一种组合情况
// 状态转移表达式：dp[n]=dp[n]+dp[n-coin]
// 等号右边的 dp[n] 表示之前得到的子解，n-coin 则表示选择了当前 coin，然后带入 dp 计算即可
...
```go
// 最长回文子串：dp[i][j]表示s[i..j]是否是一个回文串
// 回文串特性：如果 s[i:j] 是一个回文串，则去掉两边相同长度字符也一定是回文串（递归思路，问题范围由大到小）
// 反之，如果 dp[i+1][j-1] 是一个回文串，且 s[i]==s[j]，则 dp[i][j] 也是一个回文串（循环思路，问题范围由小到大）
// 即 dp[i][j]=dp[i+1][j-1],s[i]==s[j] i<=j-1。即表示 s[i]==s[j] 时，dp[i+1][j-1]
// 基本情况：长度为1时本身就是个回文串，dp[i][i]=true,
// 长度为2时，如果两个字符相等则是回文串，dp[i][i+1]=true,s[i]==s[i+1]
