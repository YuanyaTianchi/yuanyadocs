...
安装 docker：[设置存储库](https://docs.docker.com/engine/install/rhel/#set-up-the-repository)，然后[安装 Docker Engine 和 containerd](https://docs.docker.com/engine/install/rhel/#install-docker-engine) 。
...
# 安装 Docker Engine 和 containerd。centos8 默认使用 podman 代替 docker，会提示冲突，根据提示使用 --allowerasing 替换冲突的软件包
...
docker-registry 安装：使用官方 `registry` 镜像来安装执行。注意，此时启用的 registry 是一个 http server

```shell
# 仓库默认被创建在容器的 /var/lib/registry 目录下，通过 -v 挂载主机目录，以将镜像文件持久存储到主机
docker run -d --restart=always --name registry -p 5000:5000 \
-v /var/lib/docker/registry:/var/lib/registry registry
```

在 Docker client 上，默认接收 https 响应，因此还 push 不了镜像，将出现错误 `http: server gave HTTP response to HTTPS client`，可以通过在 docker **client 所在主机**的 `/etc/docker/daemon.json` 中配置取消这个限制，`192.168.31.11:5000` 为 **registry server** 地址
...
但kubelet通过cri调用时默认使用https，因此https更佳

#### https(ca)

在 registry server 主机上创建证书，指定 server [配置](../it.crypto.tool.md#签发配置)。
...
启动 https 的 registry server

```shell
# -v 挂载证书所在目录，通过环境变量指定证书和key
docker run -d --restart=always --name registry -p 5000:5000 \
...
此时 docker client 仍然无法访问私有仓库，将报错`Get "https://192.168.31.11:5000/v2/": x509: certificate signed by unknown authority`，是私有仓库使用 server 证书是我们自己生成的 ca 证书签名的自签名证书，client 主机无法对其 CA 进行验证，需要下载 ca 证书到 `/etc/docker/certs.d/<地址>:<端口>`或`/etc/docker/certs.d/<域名>`目录下
